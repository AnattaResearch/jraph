...
def get_fully_connected_graph(n_node_per_graph: int,
  ...
  if node_features is not None:
    num_node_features = tree.tree_leaves(node_features)[0].shape[0]
    if n_node_per_graph * n_graph != num_node_features:
      raise ValueError(
          'Number of nodes is not equal to num_nodes_per_graph * n_graph.')
  if global_features is not None:
    if n_graph != tree.tree_leaves(global_features)[0].shape[0]:
...
def zero_out_padding(graph: gn_graph.GraphsTuple) -> gn_graph.GraphsTuple:
  ...
  masked_edges = tree.tree_map(_get_zero_fn(edge_mask), graph.edges)
  node_mask = get_node_padding_mask(graph)
  masked_nodes = tree.tree_map(_get_zero_fn(node_mask), graph.nodes)
  global_mask = get_graph_padding_mask(graph)
  masked_globals = tree.tree_map(_get_zero_fn(global_mask), graph.globals)
